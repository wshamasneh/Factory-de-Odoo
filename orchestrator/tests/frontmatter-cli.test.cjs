 ...
 * exercised through odoo-gsd-tools.cjs via execSync.
 *
...
function writeTempFile(content) {
  const tmpFile = path.join(os.tmpdir(), `odoo-gsd-fm-test-${Date.now()}-${Math.random().toString(36).slice(2)}.md`);
...
describe('frontmatter set', () => {
  test('updates existing field', () => {
    ...
    const { extractFrontmatter } = require('../odoo-gsd/bin/lib/frontmatter.cjs');
