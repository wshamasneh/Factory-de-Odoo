<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates odoo-gsd-phase-researcher, odoo-gsd-planner, and odoo-gsd-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" init plan-phase "$PHASE")
...
**If `planning_exists` is false:** Error — run `/odoo-gsd:new-project` first.
...
5. Commit:
```bash
node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first": Display `/odoo-gsd:discuss-phase {X}` and exit workflow.
...
### Spawn odoo-gsd-phase-researcher

```bash
PHASE_DESC=$(node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
**If found:**
1. Read template: `~/.claude/odoo-gsd/templates/VALIDATION.md`
...
1. Re-run: `/odoo-gsd:plan-phase {PHASE} --research`
2. Disable Nyquist in config
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn odoo-gsd-planner Agent
...
<downstream_consumer>
Output consumed by /odoo-gsd:execute-phase. Plans need:
...
## 10. Spawn odoo-gsd-plan-checker Agent
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /odoo-gsd:discuss-phase ${NEXT_PHASE} --auto
  ```
...
- /odoo-gsd:plan-phase {X} --research — re-research first
...
- [ ] odoo-gsd-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] odoo-gsd-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] odoo-gsd-plan-checker spawned with CONTEXT.md
