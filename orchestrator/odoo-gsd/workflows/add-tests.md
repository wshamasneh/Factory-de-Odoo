...
Users currently hand-craft `/odoo-gsd:quick` prompts for test generation after each phase. This workflow standardizes the process with proper classification, quality gates, and gap reporting.
</purpose>
...
Example: `/odoo-gsd:add-tests 12 focus on edge cases` → `$PHASE_ARG=12`, `$EXTRA_INSTRUCTIONS="focus on edge cases"`

If no phase argument provided:

```
ERROR: Phase number required
Usage: /odoo-gsd:add-tests <phase> [additional instructions]
...
This command works on completed phases. Run /odoo-gsd:execute-phase first.
```
Exit.
...
```bash
node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" state-snapshot
```
...
{if bugs discovered:}
**Fix discovered bugs:** `/odoo-gsd:quick fix the {N} test failures discovered in phase ${phase_number}`
...
**Also available:**
- `/odoo-gsd:add-tests {next_phase}` — test another phase
- `/odoo-gsd:verify-work {phase_number}` — run UAT verification
