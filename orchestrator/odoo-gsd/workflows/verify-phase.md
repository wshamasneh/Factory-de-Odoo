...
<required_reading>
@~/.claude/odoo-gsd/references/verification-patterns.md
@~/.claude/odoo-gsd/templates/verification-report.md
</required_reading>
...
Use odoo-gsd-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use odoo-gsd-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" verify artifacts "$plan")
...
Use odoo-gsd-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/odoo-gsd/bin/odoo-gsd-tools.cjs" verify key-links "$plan")
...
See ~/.claude/odoo-gsd/templates/verification-report.md for complete template.
</step>
